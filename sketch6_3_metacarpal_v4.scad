...
servo();
servo_mount();
smallplate_two_holes();
//
//translate( [0,-5,0] )
//rotate([0,0,0])
//{
//    part1();
//    translate([0,-5,-4]) rotate([0,0,0]) translate([0,30,20])
//    {
//        servo_mount();
//        servo();
//    }
...
// bare cylinder to be hull'ed in pairs:
//translate( [-35,5,-8] )
//    cylinder( d=12,h=5 );
//translate( [0,-5,-8] )
//    cylinder( d=12,h=5 );
...
//    cube([20,26,3]);
//translate( [-60,20,-1] )
//        rotate([90,0,0]) rotate([ 0,90,0]) smallplate_two_holes();


module servo_mount2()
{
    translate([6,-9,-27]) // left wall
        cube([3,26,9.5]);
    translate([-6-3,-9,-27]) // right wall
        cube([3,26,9.5]);
    translate([-6-3,-9,-27-3]) // back support
        cube([12+6,3,9.5+3]);
    // mounting plates
    translate([6,-9+23,-27]) // left
    difference()
    {
        cube([11,3,9.5]);
        translate([7,1.5,4.25])
        rotate([90,90,0])
        {
            cylinder( d=6.7, h=2, $fn=6 ); // 3mm nut
            translate([0,0,-2]) cylinder( d=4, h=5, $fn=12 ); // 3mm screw
    ...
    translate([-6-11,-9+23,-27]) // left
    difference()
    {
        cube([11,3,9.5]);
        translate([7-3,1.5,4.25])
        rotate([90,90,0])
        {
            cylinder( d=6.7, h=2, $fn=6 ); // 3mm nut
            translate([0,0,-2]) cylinder( d=4, h=5, $fn=12 ); // 3mm screw
